...
- [2026-01-07]: Codex: Fixed Gemini CLI hard-fail (`ModelNotFoundError`) caused by invalid default model `gemini-3-auto` in `C:\Users\User\.gemini\settings.json`; set default model to `gemini-2.5-pro` and rewrote JSON without UTF-8 BOM (Gemini CLI rejects BOM). Verified by running `tools/gemini_mcp_smoke_test.js` (MCP initialize + tools/list + tools/call ask-gemini → “OK”).
