...
- [2026-01-04]: Ran a full dev-browser audit of all current category URLs and removed **77** additional confirmed-404 `/pl/.../` category URLs from the canonical seed lists (see `logs/not_found_urls.json` for the audit-based removal set).
- [2026-01-04]: Images: added `image_url` to the Gloorbot pipeline (scraper → worker → coordinator → CheapSkater ingest) and capped worker scaling via `MAX_SLOTS` (default `5`) to prevent extra Chrome windows.
