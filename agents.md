...
- [2026-01-07]: Codex: Root-caused recurring “ item shows as //” to **JS near-me DOM extraction mixing multi-product div.tile_group rows** (href/title from one tile + price/was from another). Confirmed via coordinator /api/v1/status showing price=131.0, was_price=977.49 for the DreamLine shower kit (so not a Cheapskater display bug). Fix: split div.tile_group by data-tile inside the JS extractor (mirrors Python extract_tile_group_products) + added regression test 	est_near_me_dom_tile_group_split.py that reproduces the mixed-record bug and now passes.
