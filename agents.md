...
- [2026-06-19]: Codex: Prepared performance-tuning sync for worker/browser pacing. Local changes plumb PerformanceSettings into slot worker env overrides, add safe-fast scraper wait defaults with Conservative rollback, enable image/font blocking and lean browser flags by default, and preserve hydration correctness with scoped early-exit checks.
- [2026-04-12]: Codex: Fixed misleading category labels caused by Lowe's faceted seed URLs being exposed as public `category_name` values. Coordinator fallback category derivation now treats numeric/value-like trailing slugs (for example `0-1-foot-long`, `1080p`, `100-pack`, `10001-to-20000-watt`) as facet values and falls back to the nearest real parent category instead. Bumped worker `__version__` to `0.12.22` in preparation for tagging `v0.12.22`.
- [2026-04-12]: Codex: Landed worker long-run recovery hardening for the next release. Heartbeats now include `tasks_completed`/`deals_sent`, worker detects coordinator `unknown_client` responses and force re-registers after resets/redeploys, supervisor mode now takes a strict single-instance lock before launching, and startup cleanup now prunes artifact directories by age plus count to keep long soak runs bounded. Bumped worker `__version__` to `0.12.21` in preparation for tagging `v0.12.21`.
- [2026-04-12]: Codex: Investigated post-`v0.12.19` "Coordinator: unreachable" regression. Live coordinator endpoints (`/healthz`, `/api/v1/status`, `/api/v1/client/register`) all returned `200`, so issue was traced to packaged worker networking. Hardened worker API requests to use `certifi.where()` explicitly when frozen and updated release build to bundle certifi data/imports with PyInstaller. Bumped worker `__version__` to `0.12.20` and tagged `v0.12.20`.
- [2026-04-12]: Codex: Fixed worker runtime crash from released `v0.12.18` (`AttributeError: 'NoneType' object has no attribute '__dict__'` at `PARALLEL/scraper.py` dataclass import). Root cause: dynamic import in `slot_worker._load_parallel_scraper()` executed module before registering it in `sys.modules`. Patched loader to register module name before `exec_module()` and clean up on failure; bumped worker `__version__` to `0.12.19` and tagged `v0.12.19`.
- [2026-04-12]: Codex: After repeated Inno Setup failures (`v0.12.14`-`v0.12.17`), switched worker release packaging to deterministic PyInstaller one-file output (`dist/GloorbotWorker.exe`) and mapped that binary directly to release asset `apps/worker/dist-installer/WorkerSetup.exe` (same download contract, no Inno dependency). Bumped worker `__version__` to `0.12.18` and tagged `v0.12.18`.
