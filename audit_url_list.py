"""
Audit a list of Lowe's URLs (typically /pl/) using the proven Akamai-resistant workflow.

Why:
- We need to compare candidate category URLs and detect redundancy using observable signals
  (product grid count + sample product IDs + breadcrumb).
- This script reuses the same approach that successfully audited the 515 URLs in LowesMap.txt.

Usage:
  python audit_url_list.py --input discovered/pl_candidates.txt --output discovered/pl_audit_results.json

Notes:
- Runs headed, real Chrome (`channel='chrome'`) and persistent profile by default.
- Writes progress continuously to JSONL for safe resume.
"""

from __future__ import annotations

import argparse
import asyncio
import json
import random
import re
from pathlib import Path
from typing import Any
from urllib.parse import urlparse

from playwright.async_api import async_playwright


BASE = "https://www.lowes.com"


def _norm(url: str) -> str:
    url = url.strip()
    if not url:
        return ""
    if url.startswith("/"):
        url = BASE + url
    parsed = urlparse(url)
    return f"{BASE}{parsed.path.rstrip('/')}"


async def human_mouse_move(page) -> None:
    viewport = page.viewport_size
    width = viewport.get("width", 1440) if viewport else 1440
    height = viewport.get("height", 900) if viewport else 900

    start_x = random.random() * width * 0.3
    start_y = random.random() * height * 0.3
    end_x = width * 0.4 + random.random() * width * 0.4
    end_y = height * 0.4 + random.random() * height * 0.4

    steps = 10 + int(random.random() * 10)
    for i in range(steps + 1):
        progress = i / steps
        eased = 2 * progress * progress if progress < 0.5 else 1 - pow(-2 * progress + 2, 2) / 2
        x = start_x + (end_x - start_x) * eased + (random.random() - 0.5) * 3
        y = start_y + (end_y - start_y) * eased + (random.random() - 0.5) * 3
        await page.mouse.move(x, y)
        await asyncio.sleep((15 + random.random() * 25) / 1000)


async def human_scroll(page) -> None:
    scroll_amount = 150 + int(random.random() * 200)
    steps = 4 + int(random.random() * 4)
    step_amount = scroll_amount / steps
    for _ in range(steps):
        await page.mouse.wheel(0, step_amount)
        await asyncio.sleep((40 + random.random() * 80) / 1000)


async def warmup_page(page) -> None:
    """Required Akamai bypass warmup."""
    await page.goto(BASE, wait_until="domcontentloaded", timeout=60000)
    await asyncio.sleep(3 + random.random() * 2)

    try:
        await human_mouse_move(page)
        await asyncio.sleep(0.8 + random.random() * 0.8)
        await human_scroll(page)
        await asyncio.sleep(1.2 + random.random() * 1.2)
        await human_mouse_move(page)
    except Exception:
        pass

    await asyncio.sleep(1.5 + random.random() * 1.5)

    # Wait for common Akamai / bot-manager cookies to appear.
    for _ in range(30):  # up to ~30s
        try:
            cookies = await page.context.cookies(BASE)
            abck = next((c for c in cookies if isinstance(c, dict) and c.get("name") == "_abck"), None)
            if abck and isinstance(abck.get("value"), str) and "~0~" in abck["value"]:
                return
        except Exception:
            pass
        await asyncio.sleep(1)


def _extract_product_id_from_href(href: str | None) -> str | None:
    if not href:
        return None
    m = re.search(r"/(\d+)(?:\?|$)", href)
    return m.group(1) if m else None


async def test_url(page: Any, url: str, index: int, total: int) -> dict[str, Any]:
    print(f"\n[{index}/{total}] Testing: {url}")
    try:
        await asyncio.sleep(1 + random.random())  # 1-2s between navigations
        await page.goto(url, wait_until="domcontentloaded", timeout=60000)
        await asyncio.sleep(1 + random.random())
        try:
            await human_mouse_move(page)
            await asyncio.sleep(0.4 + random.random() * 0.6)
            await human_scroll(page)
        except Exception:
            pass
        await asyncio.sleep(1 + random.random())

        title = ""
        for _ in range(3):
            try:
                title = await page.title()
                break
            except Exception:
                await asyncio.sleep(0.75)
        title = title or ""
        if "Access Denied" in title or "Request blocked" in title:
            print("  BLOCKED")
            return {"url": url, "blocked": True, "count": 0, "product_ids": [], "title": title}

        selectors = [
            '[data-test="product-pod"]',
            '[class*="ProductCard"]',
            '[class*="product-card"]',
            "article",
        ]
        max_count = 0
        for sel in selectors:
            try:
                count = await page.locator(sel).count()
            except Exception:
                count = 0
            if count > max_count:
                max_count = count

        product_ids: list[str] = []
        try:
            await page.locator('a[href*="/pd/"], a[href*="://www.lowes.com/pd/"]').first.wait_for(timeout=8000)
        except Exception:
            pass
        product_links = await page.locator('a[href*="/pd/"], a[href*="://www.lowes.com/pd/"]').all()
        for link in product_links[:100]:
            href = await link.get_attribute("href")
            pid = _extract_product_id_from_href(href)
            if pid and pid not in product_ids:
                product_ids.append(pid)

        if len(product_ids) < 10:
            try:
                raw_vals = await page.locator(
                    "[data-itemid], [data-item-id], [data-sku], [data-productid], [data-product-id]"
                ).evaluate_all(
                    """els => els.slice(0, 500).map(e =>
                      e.getAttribute('data-itemid') ||
                      e.getAttribute('data-item-id') ||
                      e.getAttribute('data-sku') ||
                      e.getAttribute('data-productid') ||
                      e.getAttribute('data-product-id')
                    )"""
                )
                for v in raw_vals:
                    if not v:
                        continue
                    m = re.search(r"(\d{6,})", str(v))
                    if m:
                        pid = m.group(1)
                        if pid not in product_ids:
                            product_ids.append(pid)
            except Exception:
                pass

        breadcrumb = None
        try:
            crumb = await page.locator('nav[aria-label="Breadcrumb"]').first.inner_text(timeout=2000)
            breadcrumb = " > ".join([c.strip() for c in crumb.splitlines() if c.strip()])
        except Exception:
            breadcrumb = None

        print(f"  OK: {max_count} products, {len(product_ids)} IDs extracted")
        return {
            "url": url,
            "blocked": False,
            "count": max_count,
            "product_ids": product_ids[:10],
            "title": title,
            "breadcrumb": breadcrumb,
        }
    except Exception as e:
        print(f"  Error: {e}")
        return {"url": url, "error": str(e), "blocked": False, "count": 0, "product_ids": []}


def _append_jsonl(path: Path, record: dict[str, Any]) -> None:
    with open(path, "a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False) + "\n")


def _load_existing_results(path: Path) -> dict[str, dict[str, Any]]:
    if not path.exists():
        return {}
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        if isinstance(data, list):
            return {item.get("url"): item for item in data if isinstance(item, dict) and item.get("url")}
    except Exception:
        pass
    return {}


def _load_url_list(path: Path) -> list[str]:
    urls: list[str] = []
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        line = line.strip()
        if not line or line.startswith("#"):
            continue
        u = _norm(line)
        if u:
            urls.append(u)
    # preserve order but de-dupe
    out: list[str] = []
    seen = set()
    for u in urls:
        if u not in seen:
            seen.add(u)
            out.append(u)
    return out


async def run_audit(urls: list[str], output_json: Path, output_jsonl: Path, profile_dir: Path) -> None:
    profile_dir.mkdir(parents=True, exist_ok=True)

    existing = _load_existing_results(output_json)
    if existing:
        print(f"Resuming: found {len(existing)} existing results in {output_json}")

    async with async_playwright() as p:
        context = await p.chromium.launch_persistent_context(
            str(profile_dir),
            headless=False,
            channel="chrome",
            viewport={"width": 1440, "height": 900},
            locale="en-US",
            timezone_id="America/Los_Angeles",
            args=[
                "--disable-blink-features=AutomationControlled",
                "--disable-dev-shm-usage",
                "--disable-infobars",
                "--lang=en-US",
            ],
        )
        page = context.pages[0] if context.pages else await context.new_page()

        print("Initial warmup...")
        await warmup_page(page)

        results: list[dict[str, Any]] = list(existing.values()) if existing else []
        done_ok = {r.get("url") for r in results if r.get("url") and not r.get("blocked") and "error" not in r}

        for i, url in enumerate(urls, 1):
            if url in done_ok:
                print(f"\n[{i}/{len(urls)}] Skipping already-successful: {url}")
                continue

            result = await test_url(page, url, i, len(urls))
            if result.get("blocked") or result.get("error"):
                for attempt in range(2):
                    print(f"  Retry after warmup ({attempt+1}/2)")
                    try:
                        await warmup_page(page)
                    except Exception:
                        pass
                    result = await test_url(page, url, i, len(urls))
                    if not result.get("blocked") and not result.get("error"):
                        break

            if result.get("url") in existing:
                results = [r for r in results if r.get("url") != result.get("url")]
            results.append(result)
            existing[result.get("url")] = result
            _append_jsonl(output_jsonl, result)

            if i % 25 == 0:
                output_json.write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding="utf-8")
                print(f"\nSAVED progress ({i}/{len(urls)})")

        output_json.write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding="utf-8")
        await context.close()

    blocked = sum(1 for r in results if r.get("blocked"))
    errors = sum(1 for r in results if "error" in r)
    empty = sum(1 for r in results if r.get("count", 0) == 0 and not r.get("blocked"))
    print("\n" + "=" * 70)
    print("AUDIT COMPLETE!")
    print("=" * 70)
    print(f"Total URLs tested: {len(results)}")
    print(f"Blocked: {blocked}")
    print(f"Errors: {errors}")
    print(f"Empty (0 products): {empty}")
    print(f"Success: {len(results) - blocked - errors}")
    print(f"\nResults saved to: {output_json}")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", required=True, help="Path to input URL list (one URL per line).")
    ap.add_argument("--output", required=True, help="Path to output JSON file (list of audit records).")
    ap.add_argument(
        "--profile",
        default=str(Path(".playwright-profiles") / "url-audit"),
        help="Persistent profile dir (default: .playwright-profiles/url-audit).",
    )
    args = ap.parse_args()

    in_path = Path(args.input)
    out_json = Path(args.output)
    out_jsonl = out_json.with_suffix(out_json.suffix + "l")  # .jsonl-ish
    profile_dir = Path(args.profile)

    urls = _load_url_list(in_path)
    print(f"Loaded {len(urls)} URLs from {in_path}")
    print(f"Output JSON: {out_json}")
    print(f"Output JSONL: {out_jsonl}")
    try:
        asyncio.run(run_audit(urls, out_json, out_jsonl, profile_dir))
    except KeyboardInterrupt:
        raise
    except Exception as e:
        # Make failures resumable: if JSON isn't up-to-date yet, rebuild it from JSONL.
        try:
            if out_jsonl.exists():
                rows = []
                for line in out_jsonl.read_text(encoding="utf-8", errors="ignore").splitlines():
                    line = line.strip()
                    if not line:
                        continue
                    try:
                        rows.append(json.loads(line))
                    except Exception:
                        pass
                if rows:
                    out_json.write_text(json.dumps(rows, indent=2, ensure_ascii=False), encoding="utf-8")
        except Exception:
            pass
        raise RuntimeError(f"audit_url_list failed: {e}") from e


if __name__ == "__main__":
    main()
