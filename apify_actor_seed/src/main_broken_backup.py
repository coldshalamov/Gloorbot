...
CRITICAL REQUIREMENTS FOR AKAMAI BYPASS (PROVEN WORKING):
✅ Real Chrome browser (channel='chrome') - NOT Chromium
✅ Persistent browser profile (launch_persistent_context)
✅ Homepage warmup FIRST with human behavior (mouse movements, scrolling)
✅ Human-like behavior after EVERY navigation
✅ headless=False (Akamai blocks headless)
✅ Standard launch args (--disable-blink-features=AutomationControlled)

WHAT DOES NOT WORK (causes blocks):
❌ playwright-stealth - This is a RED FLAG to Akamai!
❌ Custom fingerprint injection (Canvas/WebGL/Audio) - Makes it worse!
❌ Direct navigation to category pages - Must visit homepage first
❌ Missing human behavior - Mouse movements and scrolling required
❌ Fresh browser instances - Persistent profiles build trust

PROVEN WORKING (from test_exact_from_js.py - 100% success rate):
- Chrome + persistent profile + homepage warmup = SUCCESS
- Tested and verified on this PC, gets 29 products without blocking
